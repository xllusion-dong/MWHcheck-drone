#!/bin/bash
#version:v1.1
#20221025 add apache include conf data
...
function apache_inquiry_info(){
    ...
    do
        ...
        cat $apache_home/$apache_config|grep -vE '^#|^\s*#|^$' >> $apache_temp
        ...
        do
            if echo "$include"|grep -q -E '\.conf$'; then
                if [[ $include =~ ^/.* ]];then
                    ...
                    cat $include|grep -vE '^#|^\s*#|^$' >> $apache_temp
                else
                    echo "######$apache_home/$include Info:" >> $apache_temp
                    cat $apache_home/$include|grep -vE '^#|^\s*#|^$' >> $apache_temp
                fi
            fi
        done
        # 获取apache监听端口
        echo "##############Process $pid Apache Port Info###############"
        apache_listen=`cat $apache_temp|grep -v '^#'|grep "Listen"`
        a=":"
        if [[ "$apache_listen" =~ "$a" ]];then
            listen_port=`echo $apache_listen|awk -F ":" '{print $NF}'`
        else
            listen_port="$apache_listen"
        fi
        ...
        # 版本隐藏配置
        server_token=`cat "$apache_temp"|grep -v '^#'|grep ServerTokens|awk '{print $NF}'`
        if [ -n "$server_token" ];then
            echo "Process $pid Apache ServerToken Check Result:ServerTokens $server_token"
            ...
            echo "Process $pid Apache ServerToken Check Result:ServerTokens OS"
            echo "Process $pid Apache ServerToken Check Conclusion:Failed"
        fi
        
        # 禁止目录浏览
        echo "Process $pid Apache Directory OptionsCheckInfo"
        cat $apache_temp|grep -v '#'|grep -v IndexOptions|grep Options|sed 's/^[ \t]*//g'
        echo "Process $pid Apache Directory OptionsCheckInfoEND"
        indexes_1=`cat $apache_temp|grep -v '#'|grep -v IndexOptions|grep Options|grep " Indexes"`
        indexes_2=`cat $apache_temp|grep -v '#'|grep -v IndexOptions|grep Options|grep "+Indexes"`
        if [[ -n $indexes_1 || -n $indexes_2 ]];then
            echo "Process $pid Apache Directory Options Check Conclusion:Failed"
        else
            echo "Process $pid Apache Directory Options Check Conclusion:Pass"
        fi

        # 自定义错误页面检查
        echo "Process $pid Apache Error PagesCheckInfo"
        error_page=`cat $apache_temp|grep -v '#'|grep ErrorDocument|sed '/^$/d'`
        echo "$error_page"
        echo "Process $pid Apache Error PagesCheckInfoEND"
        ...
        echo "Process $pid Apache Config Info:" && cat $apache_temp
...
function get_apache_data(){
    ...
    directory_check=`cat $file1|sed -n /OptionsCheckInfo/,/OptionsCheckInfoEND/p|grep -v "OptionsCheckInfo"|grep -v "OptionsCheckInfoEND"` && echo "\"directory_check\"":"\"$directory_check\"""," >> $new_document
    directory_check_con=`cat $file1|grep "Apache Directory Options Check Conclusion:"|awk -F ":" '{print $2}'` && echo "\"directory_check_con\"":"\"$directory_check_con\"""," >> $new_document
    errorpage_check=`cat $file1|sed -n /PagesCheckInfo/,/PagesCheckInfoEND/p|grep -v "PagesCheckInfo"|grep -v "PagesCheckInfoEND"`
    if [ -n "$errorpage_check" ];then
        echo "\"errorpage_check\"":"\"$errorpage_check\"""," >> $new_document
    else
        echo "\"errorpage_check\"":"\"Null-没有配置错误页面\"""," >> $new_document
    fi
