...
function tomcat_info(){
	...
	for OPID in $PID; do
		...
		tomcat_path=`ls -l /proc/$OPID/cwd|awk -F "->" '{print $2}'|tr -d ' '|awk -F "/bin" '{print $1}'`
		...
		# AJP端口检查
		ajp_check=`cat $tempxml|grep "<Connector"|grep "AJP"`
		if [[ -n $ajp_check ]];then
			echo "Process $OPID Tomcat AJP Check Result:Failed"
		else
			echo "Process $OPID Tomcat AJP Check Result:Pass"
		fi
		# AccessLog完备性检查
		log_check1=`cat $tempxml|grep "User-Agent"`
		log_check2=`cat $tempxml|grep "Referer"`
		if [[ -z $log_check1 || -z $log_check2 ]];then
			echo "Process $OPID Tomcat Accesslog Check Result:Failed"
		else
			echo "Process $OPID Tomcat Accesslog Check Result:Pass"
		fi

		# 用户锁定检查
		if [ -e $userxml ];then
			user_check=`sed 's/<!--.*-->//g' $userxml|sed -n '/<!--/,/-->/!p'|grep "username="`
			if [[ -n $user_check ]];then
				echo "Process $OPID Tomcat Userxml Check Result:Failed"
			else
				echo "Process $OPID Tomcat Userxml Check Result:Pass"
			fi
		else
			echo "Process $OPID Tomcat Userxml Check Result:Pass" 
		fi
		...
		if [[ $list_check = "false" ]];then
			echo "Process $OPID Tomcat Listings Check Result:Pass"
		else
			echo "Process $OPID Tomcat Listings Check Result:Failed"
		fi
		# 文件上传漏洞防御检查
		putstr=`cat $webxml|grep ">readonly<"`
		if [ -n $putstr ];then
			put_check=`cat $webxml |grep -A1 ">readonly<"|grep -v ">readonly<"|awk -F ">" '{print $2}'|awk -F "<" '{print $1}'`
			if [[ $put_check = "false" ]];then
				echo "Process $OPID Tomcat FilePut Check Result:Pass"
			else
				echo "Process $OPID Tomcat FilePut Check Result:Failed"
			fi
		else
			echo "Process $OPID Tomcat FilePut Check Result:Failed"
		fi
		# 删除临时生成文件
		rm -f $tempxml
		echo "#############View Process $OPID Tomcat Resource Usage##########"
		top -bn1|grep "%CPU %MEM"
		top -bn1|grep $OPID
		echo "#############View Process $OPID Tomcat Use Ports###############" 
		netstat -nutlp | grep $OPID
		if [[ ${jdk_path} =~ "jre" ]];then
			echo "###########Process $OPID Tomcat Running With JRE！############"
		else
			echo "##############View Process $OPID Tomcat GC Info################"  
			$jdk_path/bin/jstat  -gc  $OPID 250 4   
			echo "##########View Process $OPID Tomcat JVM Memory Stats###########"  
			$jdk_path/bin/jstat -gccapacity $OPID  
			echo "##########View Process $OPID Tomcat Classload Info#############"  
			$jdk_path/bin/jstat -class  $OPID
		fi
...
function get_tomcat_jsondata(){
    ...
    tomcat_version=`cat $file1|grep "Server number:"|grep -v log|awk -F ':' '{print $2}'|tr -d " "` && echo "\"tomcat_version\"":"\"$tomcat_version\"""," >> $new_document
	...
	jvm_gc=`cat $file1|grep -A5 "Tomcat GC Info"` && echo "\"jvm_gc\"":"\"$jvm_gc\"""," >> $new_document
	jvm_classload=`cat $file1|grep -A2 "Tomcat Classload Info"` && echo "\"jvm_classload\"":"\"$jvm_classload\"""," >> $new_document
	resource_use=`cat $file1|grep -A2 "Resource Usage"` && echo "\"resource_use\"":"\"$resource_use\"""," >> $new_document
	...
	apps_check=`cat $file1|grep "Tomcat Applications:"|awk -F ":" '{print $2}'` && echo "\"apps_check\"":"\"$apps_check\"""," >> $new_document
...
get_os_jsondata
get_tomcat_jsondata
tar -zcf /tmp/enmoResult/"$HOSTNAME"_"tomcat"_"$ipinfo"_"$qctime".tar.gz /tmp/enmoResult/* --exclude=/tmp/enmoResult/*.tar.gz