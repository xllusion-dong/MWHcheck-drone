...
#update: 增加对版本的判断，降低因为版本导致的命令不一致，产生的错误输出
#update: 修改输出的格式[1m [0m内容
#update-date:2022-11-7
...
function get_rabbitmq_main() {
    ...
    if [ -f $rabbitmq_sbin/rabbitmqctl  ];then
        date=$(date +%Y%m%d)
        ...
        if [  $? -eq 0 ];then
            date=$(date +%Y%m%d)
            rabbitmqctl_dir=`which rabbitmqctl`
            rabbitmq_sbin=`dirname $rabbitmqctl_dir`
            check_rabbitmq_command_line >>"$filepath""$filename2"
            get_log >>"$filepath""$filename2"
            get_config >>"$filepath""$filename2"
            rabbitmq_security_baseline >>"$filepath""$filename2"
            ...
            echo "not find rabbitmq sbin directory,The script continues to execute and some data will be missing"
...
new_document_1=$new_data/rabbitmq-new_$ip_1.json
esc=$'\e'
sed "s/$esc\[[0-9;]*m//g" $new_document > $new_document_1
rm -rf $new_document
cd $data_path
