...
function  rocketmq_broker_log(){
    ...
    if [ $rocketmq_namesvr_member -gt 0 ];then
        ...
        do
            ...
            namesvrlogpath=$(ps -feww | grep $line  | grep -v grep | grep -io "rocketmq.client.logRoot=.*" | awk '{FS=" "; print $1}' | cut -d "=" -f2)
            if [ -z "$namesvrlogpath" ];then
                namesvrlogconfigfile=$rocketmqBase/conf/logback_namesrv.xml
                namesvrlogpathfile=$(grep 'file' $namesvrlogconfigfile | awk 'BEGIN{FS=">";RS="</"}{print $NF}' | sed '/^\(\s\)*$/d'  | grep namesrv.log)
                namesvrlogpath=$(echo ${namesvrlogpathfile%/*})
                if [[ $namesvrlogpath == *user.home* ]];then
                    #获取当前用户的家目录
...
if [ $rocketmq_pid_member -eq 0 ]; then
    if [ $rocketmq_namesvr_member -eq 0 ]; then
        ...
        do
            runUser=$(ps -eo ruser,pid | grep $line | awk '{print $1}')
            namesvrlogpath=$(ps -feww | $line  | grep -v grep | grep -io "rocketmq.client.logRoot=.*" | awk '{FS=" "; print $1}' | cut -d "=" -f2)
            if [ -z "$namesvrlogpath" ];then
                namesvrlogconfigfile=$rocketmqBase/conf/logback_namesrv.xml
                namesvrlogpathfile=$(grep 'file' $namesvrlogconfigfile | awk 'BEGIN{FS=">";RS="</"}{print $NF}' | sed '/^\(\s\)*$/d'  | grep namesrv.log)
                namesvrlogpath=$(echo ${namesvrlogpathfile%/*})
