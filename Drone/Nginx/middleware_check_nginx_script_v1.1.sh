...
function nginx_inquiry_info(){
	...
	do
		...
		nginx_dir=`ls -l /proc/$NPID/cwd|awk '{print $(NF)}'|sed s/sbin//g`
...
get_os_jsondata
get_nginx_data
tar -zcf /tmp/enmoResult/"$HOSTNAME"_"nginx"_"$ipinfo"_"$qctime".tar.gz --exclude=/tmp/enmoResult/*.tar.gz --format=ustar /tmp/enmoResult/*