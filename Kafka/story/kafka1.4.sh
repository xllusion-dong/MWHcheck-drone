...
function kafka_security_baseline() {
    ...
    for line in $(ps -ef | grep kafka.Kafka | grep -v grep | awk -F " " '{print $2}'); do
        ...
        security_baseline_port=$(cat $kafka_config_dir/server.properties | grep listeners= | grep -v "#" |grep -v "advertised"| awk -F ":" '{print $NF}') && echo "security_baseline_port:$security_baseline_port"
        if [ $kafka_port_member == 0 ];then
            security_baseline_port1=9092
...
function get_kafka_data() {
    ...
    for line in $(ps -ef | grep kafka.Kafka | grep -v grep | awk -F " " '{print $2}'); do
        ...
        Start_User=$(cat $file1 |grep $line | grep -A1 "当前服务器Kafka进程地址(供参考)：" | grep -v "当前服务器Kafka进程地址(供参考)：" | awk -F " " '{print $1}') && echo "\"Start_User\"":"\"$Start_User\"""," >>$new_document
        JVM_Instal_path=$(cat $file1 |grep $line | grep -A1 "当前服务器Kafka进程地址(供参考)：" | grep -v "当前服务器Kafka进程地址(供参考)：" | awk -F " " '{print $8}') && echo "\"JVM_Instal_path\"":"\"$JVM_Instal_path\"""," >>$new_document
...
function check_value_quick_output(){
    ...
    do    
        ...
        if [ $Kafka_Version -gt "220" ]; then
            ...
            I=0;
            for line in $($kafka_bin/kafka-consumer-groups.sh --bootstrap-server $kafka_ip:$kafka_port --describe --all-groups|awk -F " " '{print $6}'|grep -v "LAG"|sed 's/[^0-9]//g');
            do
                I=$(($I + $line));
            done
            if [ $I -ge 5000 ];then
            ...
            do 
                echo "判断标准:获取消息积压数，超过5000输出UnHealthy" >> $Kafka_Log_File
                I=0;
                for line in $($kafka_bin/kafka-consumer-groups.sh --bootstrap-server $kafka_ip:$kafka_port--describe --group $i|awk -F " " '{print $6}'|grep -v "LAG"|sed 's/[^0-9]//g');
                do
                    I=$(($I + $line));
                done
                if [ $I -ge 5000 ];then
                ...
                #memeber数据，是否为empty还是stable，如果没有empty 为Health
                #2.2版本以下kafka-consumer-groups.sh缺少-state参数，所以如下部分进行注释，不再做输出
                #Kafka_Group_State=`$kafka_bin/kafka-consumer-groups.sh --zookeeper $zookeeper_ip_port  --describe --group $i --state|grep -v "COORDINATOR"|awk -F " " '{print $4}'`
                #if [ `echo  $line|grep -i "Empty"|wc -l ` -ne 0 ];then
                #    echo "Kafka-Group-State:unHealthy" >> $Kafka_Log_File
                #else
                #    echo "Kafka-Group-State:Health" >> $Kafka_Log_File
