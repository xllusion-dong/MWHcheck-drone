...
#update:新增对巡检数据是否成功获取的校验，避免空数据，影响报告生成；增加数据快速判断结果，方便当下结论输出；
#update-date:2022-12-28
#update:0.11、1.0/1.1/1.11版本  巡检脚本报无--report-format参数，增加对应版本判断；
#update:新增在脚本运行初期，就对kafka进程进行判断，在无进程的情况下，直接中断脚本；
#update:新增对同一台服务器，多个进程的数据采集
#update-date:2023-2-7
#update:修复不同版本调用kafka命令没有对应option参数及单机多进程获取数据报错等bug
#update-date:2023-2-27
#update:修复不同版本调用kafka命令没有对应option参数及增加对unhealthy的说明
#update-date:2023-5-5
#update:修复2.4版本以下kafka，无法正常获取消费者动态信息错误
#update-date:2023-5-30
#update:新增脚本对kraft模式的支持
...
function collect_sys_info() {
    ...
    echo "Kafka-OS-Mem-Used:"
    Mem_Rate=$(awk 'BEGIN{printf"%.2f\n",'$MemUsed' / '$MemTotal' *100}') #保留小数点后2位
    Mem_Rate1=$(awk 'BEGIN{printf"%.0f\n",'$MemUsed' / '$MemTotal' *100}')  #保留整数
    ...
    Diskutil1=$(awk 'BEGIN{printf"%.0f\n",'$Usesum' / '$Totalsum'*100}')
    Freeutil=$(awk 'BEGIN{printf"%.2f\n",100 - '$Diskutil'}')
    Freeutil1=$(awk 'BEGIN{printf"%.0f\n",100 - '$Diskutil1'}')
...
function kafka_inquiry_info() {
    ...
    for kafka_pid in `ps -ef | grep kafka.Kafka | grep -v grep | awk -F " " '{print $2}'`
    do    
        ...
        kafka_ip_member=$(cat $kafka_config | grep listeners=PLAINTEXT:// | grep -v ^# |grep -v "advertised"| awk -F "//" '{print $2}' | awk -F ":" '{print $1}' | grep -v '^$' | wc -l)
        if [ $kafka_ip_member == 0 ]; then
            kafka_ip=$ip
        else
            kafka_ip=$(cat $kafka_config | grep listeners=PLAINTEXT:// | grep -v ^# |grep -v "advertised"| awk -F "//" '{print $2}' | awk -F ":" '{print $1}' | grep -v '^$')
        fi
        kafka_port_member=$(cat $kafka_config | grep listeners=PLAINTEXT:// | grep -v ^# |grep -v "advertised"| awk -F "//" '{print $2}' | awk -F ":" '{print $2}' | grep -v '^$'|wc -l)
        if [ $kafka_port_member == 0 ];then
            kafka_port=9092
        else
            kafka_port=$(cat $kafka_config | grep listeners=PLAINTEXT:// | grep -v ^# |grep -v "advertised"| awk -F "//" '{print $2}' | awk -F ":" '{print $2}' | grep -v '^$')
        fi    
        ...
        if [ $Kafka_Version -gt "240" ]; then
            ...
            echo ""
            echo "================查询Kafka动静态配置======================"
            if [ $Kafka_Version -eq "250" ] || [ $Kafka_Version -eq "251" ];then
            ...
            #Kafka开启JMX的地方可以有多个，在kafka-run-class.sh中、kafka-server-start.sh还有env export形式指定，所以需要多个地方进行判断
            cd $kafka_bin
            jmx_port_memeber=$(grep -i "JMX_port" --exclude-dir windows  *|grep -v "#"|grep -v "if" |grep -v "KAFKA_JMX_OPTS"|grep -v grep|wc -l)
    
            if [ $jmx_port_memeber == 0 ]; then
                echo "暂未开启JMX端口，不做mebean数据采集"
            else
                jmx_port=$(grep -i "JMX_port" --exclude-dir windows  *|grep -v "#"|grep -v "if" |grep -v "KAFKA_JMX_OPTS"|grep -v grep|awk -F "=" '{print $NF}'|sed 's/\"//g')
        ...
        echo "开始进行日志查询操作，当前查询节点为$kafka_ip-$kafka_port "
            ...
            echo ""
            echo "================查询集群描述======================"
            kafka_topic_list=`$kafka_bin/kafka-topics.sh --list --zookeeper $zookeeper_ip_port`
            for i in $kafka_topic_list;do
                echo "================当前Topic描述为：$i======================"
                $kafka_bin/kafka-topics.sh --describe --zookeeper $zookeeper_ip_port --topic $i
            done
            echo ""
            echo "================查询topic列表======================"
            ...
            #$kafka_bin/kafka-configs.sh --bootstrap-server $kafka_ip:$kafka_port --entity-type brokers --describe
            echo ""
        fi
...
function kafka_security_baseline() {
    ...
    for line in $(ps -ef | grep kafka.Kafka | grep -v grep | awk -F " " '{print $2}'); do
        kafka_bin=$(pwdx $line | awk -F ":" '{print $2}')
        kafka_dir=$(dirname $kafka_bin)
        kafka_config="$kafka_dir/config/server.properties"
        kafka_config_dir="$kafka_dir/config"
        kafka_bin="$kafka_dir/bin"
        ...
        security_baseline_port=$(cat $kafka_config_dir/server.properties | grep listeners= | grep -v "#" |grep -v "advertised"| awk -F ":" '{print $NF}')  
        if [ $kafka_port_member == 0 ];then
            security_baseline_port=9092
            security_baseline_port1="UnHealthy"
            echo "security_baseline_port:$security_baseline_port"
            echo "security_baseline_port1:$security_baseline_port1"
        else
            if [ $security_baseline_port -ne 9092 ]; then
                security_baseline_port=$security_baseline_port
                security_baseline_port1="Health"
                echo "security_baseline_port:$security_baseline_port"
                echo "security_baseline_port1:$security_baseline_port1"
            else
                security_baseline_port1="UnHealthy"
                echo "security_baseline_port:9092"
                echo "security_baseline_port1:$security_baseline_port1"
            fi
        fi 
        echo "基线2:kafka的非特权用户运行。"
        security_baseline_root=$(ps -ef | grep kafka.Kafka |grep $line| grep -v grep | awk -F " " '{print $1}') && echo "security_baseline_root:$security_baseline_root"
        if [ $security_baseline_root == "root" ]; then
            security_baseline_root1="UnHealthy"
            echo "security_baseline_root1:$security_baseline_root1"
        else
            security_baseline_root1="Health"
            echo "security_baseline_root1:$security_baseline_root1"
        fi
        ...
        if [ $security_baseline_dir == "root" ]; then
            security_baseline_dir1="UnHealthy"
            echo "security_baseline_dir1:$security_baseline_dir1"
        else
            security_baseline_dir1="Health"
            echo "security_baseline_dir1:$security_baseline_dir1"
        fi
        ...
        if [ $security_baseline_partitions -eq 1 ]; then
            security_baseline_partitions1="UnHealthy"
            echo "security_baseline_partitions1:$security_baseline_partitions1"
        else
            security_baseline_partitions1="Health"
            echo "security_baseline_partitions1:$security_baseline_partitions1"
        fi
        echo "基线5:kafka是否允许自动创建topic，建议为false。"
        security_baseline_atuocreate=$(cat $kafka_config_dir/server.properties | grep "auto.create.topics" | awk -F "=" '{print $NF}')
        if [[ $security_baseline_atuocreate == "false" || $security_baseline_atuocreate == "" ]]; then
            security_baseline_atuocreate="false"
            security_baseline_atuocreate1="Health"
            echo "security_baseline_atuocreate:$security_baseline_atuocreate"
            echo "security_baseline_atuocreate1:$security_baseline_atuocreate1"
        else
            security_baseline_atuocreate="true"
            security_baseline_atuocreate1="UnHealthy"
            echo "security_baseline_atuocreate:$security_baseline_atuocreate"
            echo "security_baseline_atuocreate1:$security_baseline_atuocreate1"
        fi
        ...
        if [ $result -eq 1 ]; then
            security_baseline_logdir1="UnHealthy"
            echo "security_baseline_logdir1:$security_baseline_logdir1"
        else
            security_baseline_logdir1="Health"
            echo "security_baseline_logdir1:$security_baseline_logdir1"
        fi
        ...
        if [ $security_baseline_acl != 0 ]; then
            security_baseline_acl1="Health"
            echo "security_baseline_acl1:$security_baseline_acl1"
        else
            security_baseline_acl1="UnHealthy"
            echo "security_baseline_acl1:$security_baseline_acl1"
        fi
    done
}
...
function get_kafka_main() {
    ...
    ####get system info
    if [ `ps -ef|grep java|grep kafka|grep server.properties|grep -v grep|wc -l` -eq 0 ];then
        echo "No kafka process exists on the current server！！！"
        echo "Only check OS INFO !!!"
...
function get_kafka_data() {
    ...
    for line in $(ps -ef | grep kafka.Kafka | grep -v grep | awk -F " " '{print $2}'); do
        Kafka_version=$(cat $file1 |grep $line | grep -io "kafka_.*" | awk -F ":" '{print $1}' | awk -F "_" '{print $2}' | cut -d "." -f1-4) && echo "\"Kafka_version\"":"\"$Kafka_version\"""," >>$new_document
        Installation_path=$(cat $file1 |grep $line | grep -io 'Dkafka.logs.dir=.*' | awk -F "=" '{print $2}' | cut -d "." -f1) && echo "\"Installation_path\"":"\"$Installation_path\"""," >>$new_document
        Start_User=$(cat $file1 |grep $line | awk -F " " '{print $1}'|head -n 1) && echo "\"Start_User\"":"\"$Start_User\"""," >>$new_document
        JVM_Instal_path=$(cat $file1 |grep $line  | awk -F " " '{print $8}'|head -n 1) && echo "\"JVM_Instal_path\"":"\"$JVM_Instal_path\"""," >>$new_document
        JVM_run_parameters=$(cat $file1 |grep $line | grep -io 'Xms.*' | awk -F "-Djava" '{print $1}') && echo "\"JVM_run_parameters\"":"\"$JVM_run_parameters\"""," >>$new_document
...
function get_kafka_security_baseline() {
    new_document=$new_data/kafka_$ip_1.json
    for line in $(ps -ef | grep kafka.Kafka | grep -v grep | awk -F " " '{print $2}'); do
        Security_Baseline_Port=$(cat $file1 | grep -A 21 "\-$line" | grep security_baseline_port |grep -v security_baseline_port1 | awk -F ":" '{print $2}') && echo "\"Security_Baseline_Port\"":"\"$Security_Baseline_Port\"""," >>$new_document
        Security_Baseline_Root=$(cat $file1 | grep -A 21 "\-$line" | grep security_baseline_root | grep -v security_baseline_root1 | awk -F ":" '{print $2}') && echo "\"Security_Baseline_Root\"":"\"$Security_Baseline_Root\"""," >>$new_document
        Security_Baseline_Dir=$(cat $file1 | grep -A 21 "\-$line" | grep security_baseline_dir  | grep -v security_baseline_dir1 | awk -F ":" '{print $2}') && echo "\"Security_Baseline_Dir\"":"\"$Security_Baseline_Dir\"""," >>$new_document
        Security_Baseline_Partitions=$(cat $file1 | grep -A 21 "\-$line" | grep security_baseline_partitions | grep -v security_baseline_partitions1 | awk -F ":" '{print $2}') && echo "\"Security_Baseline_Partitions\"":"\"$Security_Baseline_Partitions\"""," >>$new_document
        Security_Baseline_Atuocreate=$(cat $file1 | grep -A 21 "\-$line" | grep security_baseline_atuocreate  | grep -v security_baseline_atuocreate1 | awk -F ":" '{print $2}') && echo "\"Security_Baseline_Atuocreate\"":"\"$Security_Baseline_Atuocreate\"""," >>$new_document
        Security_Baseline_Logdir=$(cat $file1 | grep -A 21 "\-$line" | grep security_baseline_logdir  | grep -v security_baseline_logdir1 | awk -F ":" '{print $2}') && echo "\"Security_Baseline_Logdir\"":"\"$Security_Baseline_Logdir\"""," >>$new_document
        Security_Baseline_Acl=$(cat $file1 | grep -A 21 "\-$line" | grep security_baseline_acl | grep -v security_baseline_acl1 | awk -F ":" '{print $2}') && echo "\"Security_Baseline_Acl\"":"\"$Security_Baseline_Acl\"""," >>$new_document
    
        Security_Baseline_Port1=$(cat $file1 | grep -A 21 "\-$line" | grep security_baseline_port1  | awk -F ":" '{print $2}') && echo "\"Security_Baseline_Port1\"":"\"$Security_Baseline_Port1\"""," >>$new_document
        Security_Baseline_Root1=$(cat $file1 | grep -A 21 "\-$line" | grep security_baseline_root1  | awk -F ":" '{print $2}') && echo "\"Security_Baseline_Root1\"":"\"$Security_Baseline_Root1\"""," >>$new_document
        Security_Baseline_Dir1=$(cat $file1 | grep -A 21 "\-$line" | grep security_baseline_dir1 | awk -F ":" '{print $2}') && echo "\"Security_Baseline_Dir1\"":"\"$Security_Baseline_Dir1\"""," >>$new_document
        Security_Baseline_Partitions1=$(cat $file1 | grep -A 21 "\-$line" | grep security_baseline_partitions1 | awk -F ":" '{print $2}') && echo "\"Security_Baseline_Partitions1\"":"\"$Security_Baseline_Partitions1\"""," >>$new_document
        Security_Baseline_Atuocreate1=$(cat $file1 | grep -A 21 "\-$line" | grep security_baseline_atuocreate1 | awk -F ":" '{print $2}') && echo "\"Security_Baseline_Atuocreate1\"":"\"$Security_Baseline_Atuocreate1\"""," >>$new_document
        Security_Baseline_Logdir1=$(cat $file1 | grep -A 21 "\-$line" | grep security_baseline_logdir1 | awk -F ":" '{print $2}') && echo "\"Security_Baseline_Logdir1\"":"\"$Security_Baseline_Logdir1\"""," >>$new_document
        Security_Baseline_Acl1=$(cat $file1 | grep -A 21 "\-$line" | grep security_baseline_acl1 | awk -F ":" '{print $2}') && echo "\"Security_Baseline_Acl1\"":"\"$Security_Baseline_Acl1\"" >>$new_document
    done
}
Kafka_Log_File="/tmp/enmoResult/tmpcheck/Kafka_Emporary_Files.txt"

function check_script_value_exist(){
    #检查kafka在数据采集过程中，收集的数据是否存在
    Kafka_version_member=$(cat $file1 | grep -io "kafka_.*" | awk -F ":" '{print $1}' | awk -F "_" '{print $2}' | cut -d "." -f1-4|wc -l) 
    Broker_id_member=$(cat $file1 | grep "broker.id" | grep -v "generation" | grep -v "sensitive"|wc -l)
    Topic_list=$(cat $file1 | sed -n /查询topic列表/,/新消费者列表查询/p | grep -v 新消费者列表查询 | grep -v 查询topic列表|grep -v "__consumer_offsets" |wc -l)
    echo "====================================check_script_value_exist===================================" >> $Kafka_Log_File
    if [ $Kafka_version_member -ne 0 -a $Broker_id_member -ne 0 -a $Topic_list -ne 0 ];then
        echo "Kafka script value exist success!!!" >> $Kafka_Log_File
    else
        echo "Kafka script value exist UnHealthy!!！" >> $Kafka_Log_File
        echo "Human interventions!!!" >> $Kafka_Log_File
    fi

}


function check_value_quick_output(){
    echo "====================================check_value_quick_output===================================" >> $Kafka_Log_File
    for kafka_pid in `ps -ef | grep kafka.Kafka | grep -v grep | awk -F " " '{print $2}'`
    do    
        kafka_bin=$(pwdx $kafka_pid | awk -F ":" '{print $2}')
        kafka_dir=$(dirname $kafka_bin)
        ...
        #增加数据快速判断结果，方便当下结论输出；
        #如下信息是针对kafka-os部分
        echo "====================================Kafka-$kafka_pid===================================" >> $Kafka_Log_File
        echo "IP:"$IP_Address >> $Kafka_Log_File
        echo "" >> $Kafka_Log_File
        echo "PORT:"$kafka_port >> $Kafka_Log_File
        echo "" >> $Kafka_Log_File
        echo "判断标准:当前服务器内存使用率大于90%为UnHealthy" >> $Kafka_Log_File
        if [ $Mem_Rate1 -ge "90" ];then 
            echo "Mem_Rate:UnHealthy" >> $Kafka_Log_File
            echo "" >> $Kafka_Log_File
        else
            echo "Mem_Rate:Health" >> $Kafka_Log_File
            echo "" >> $Kafka_Log_File
        fi
        #判断当前服务器磁盘剩余量是否大于20%
        echo "判断标准:当前服务器剩余磁盘空间大于20%为Healthy" >> $Kafka_Log_File
        if [ $Freeutil1 -gt "20" ];then
            echo "Disk_Freeutil:Health" >> $Kafka_Log_File
            echo "" >> $Kafka_Log_File
        else
            echo "Disk_Freeutil:UnHealthy" >> $Kafka_Log_File
            echo "" >> $Kafka_Log_File
        fi
        #判断当前服务器Openfile是否大于1024
        echo "判断标准:当前服务器Openfile大于1024为Healthy" >> $Kafka_Log_File
        if [ $Openfile -gt "1024" ];then
            echo "Openfile:Health" >> $Kafka_Log_File
            echo "" >> $Kafka_Log_File
        else
            echo "Openfile:UnHealthy" >> $Kafka_Log_File
            echo "" >> $Kafka_Log_File
        fi
    
        #如下信息是针对kafka-security部分
        echo "判断标准:9092为UnHealthy">> $Kafka_Log_File
        echo "Kafka-Security-Baseline-Port:"$security_baseline_port1 >> $Kafka_Log_File
        echo "" >> $Kafka_Log_File
        echo "判断标准:kafka使用非root用户启动为Healthy" >> $Kafka_Log_File
        echo "Kafka-Security-Baseline-Root:"$security_baseline_root1 >> $Kafka_Log_File
        echo "" >> $Kafka_Log_File
        echo "判断标准:kafka的目录权限是root用户为UnHealthy" >> $Kafka_Log_File
        echo "Kafka-Security-Baseline-Dir:"$security_baseline_dir1 >> $Kafka_Log_File
        echo "" >> $Kafka_Log_File
        echo "判断标准:kafka默认分区数，分区数是1则为UnHealthy" >> $Kafka_Log_File
        echo "Kafka-Security-Baseline-Partitions:"$security_baseline_partitions1 >> $Kafka_Log_File
        echo "" >> $Kafka_Log_File
        echo "判断标准:kafka是否允许自动创建topic，为false则为Healthy" >> $Kafka_Log_File
        echo "Kafka-Security-Baseline-Autocreate:"$security_baseline_atuocreate1 >> $Kafka_Log_File
        echo "" >> $Kafka_Log_File
        echo "判断标准:kafka的log-dir目录是否在tmp目录下，在tmp目录下为UnHealthy" >> $Kafka_Log_File
        echo "Kafka-Security-Baseline-Logdir:"$security_baseline_logdir1 >> $Kafka_Log_File
        echo "" >> $Kafka_Log_File
        echo "判断标准:kafka的授权访问，是否有acl认证，有则是Healthy" >> $Kafka_Log_File
        echo "Kafka-Security-Baseline-Acl:"$security_baseline_acl1 >> $Kafka_Log_File
        echo "" >> $Kafka_Log_File
        #如下信息是针对kafka-runtime部分
        #LAG和STATE因涉及到--all-groups参数，需要对版本信息做出判断，具体脚本可以参考如上针对307行左右
        #需要增加对Kafka版本的逻辑判断，在高于2.2版本的走原有的逻辑，低于2.2版本的，采用遍历所有的--group来获取对应值
        if [ $Kafka_Version -gt "220" ]; then
            echo "判断标准:获取消息积压数，超过5000输出UnHealthy" >> $Kafka_Log_File
            I=0;
            for line in $($kafka_bin/kafka-consumer-groups.sh --bootstrap-server $kafka_ip:$kafka_port --describe --all-groups|awk -F " " '{print $5}'|grep -v "LAG"|sed 's/[^0-9]//g');
            do
                I=$(($I + $line));
            done
            if [ $I -ge 5000 ];then
                echo "Kafka-LAG:UnHealthy" >> $Kafka_Log_File
                echo "" >> $Kafka_Log_File
            else
                echo "Kafka-LAG:Health" >> $Kafka_Log_File
                echo "" >> $Kafka_Log_File
            fi
            echo "判断标准:memeber数据，是否为empty还是stable，如果没有empty 为Health"  >> $Kafka_Log_File
            Kafka_Group_State=`$kafka_bin/kafka-consumer-groups.sh --bootstrap-server $kafka_ip:$kafka_port  --describe --all-groups --state|grep -v "COORDINATOR"|awk -F " " '{print $4}'`
            if [ `echo  $line|grep -i "Empty"|wc -l ` -ne 0 ];then
                echo "Kafka-Group-State:UnHealthy" >> $Kafka_Log_File
                echo "" >> $Kafka_Log_File
            else
                echo "Kafka-Group-State:Health" >> $Kafka_Log_File
                echo "" >> $Kafka_Log_File
            fi
        else
            echo "判断标准:获取消息积压数，超过5000输出UnHealthy" >> $Kafka_Log_File
            for i in $($kafka_bin/kafka-consumer-groups.sh --bootstrap-server $kafka_ip:$kafka_port --list);
            do  
                I=0;
                for line in $($kafka_bin/kafka-consumer-groups.sh --bootstrap-server $kafka_ip:$kafka_port --describe --group $i|awk -F " " '{print $5}'|grep -v "LAG"|sed 's/[^0-9]//g');
                do
                    I=$(($I + $line));
                done
            done
        ...
        echo "判断标准:日志文件中如果包含有error，就输出一个unHealthy" >> $Kafka_Log_File
        if [ $(cat $file1 | sed -n /开始进行日志查询操作/,/END!!!/p | grep -v "开始进行日志查询操作" | grep -v "END!!!"|grep "error" | wc -l) -eq "0" ]; then
            echo "Kafka-Log-Health":"Health" >> $Kafka_Log_File
        else
            echo "Kafka-Log-Health":"unHealthy" >> $Kafka_Log_File
        fi
        echo "===============================================================================================" >> $Kafka_Log_File
    done
}
echo "Start Kafka Date Extraction!!!" >>$Kafka_Log_File
    get_os_jsondata
    get_kafka_data
    get_kafka_security_baseline
    check_script_value_exist
    check_value_quick_output
    cd $data_path
    tar -czvf /tmp/enmoResult/"$HOSTNAME"_"kafka"_"$ipinfo"_"$qctime".tar.gz /tmp/enmoResult
    cat $Kafka_Log_File 