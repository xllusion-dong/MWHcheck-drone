...
function kafka_inquiry_kraft_info() {
    ip=$(ip addr show | grep "inet " | grep -v 127.0.0. | head -1 | cut -d" " -f6 | cut -d/ -f1)
    date=$(date +%Y-%m)
    echo "|+++++++++++++++++++++++++++++++++++++++++++++++++++++++++|"
    echo "|                kafka kraft info                         |"
    echo "|+++++++++++++++++++++++++++++++++++++++++++++++++++++++++|"
    echo ""
    echo "当前服务器Kafka进程地址(供参考)：" && ps -ef | grep kafka.Kafka | grep -v grep
    for kafka_pid in `ps -ef | grep kafka.Kafka | grep -v grep | awk -F " " '{print $2}'|head -n 1`
    do
    	kafka_bin=$(pwdx $kafka_pid | awk -F ":" '{print $2}')
        kafka_dir=$(dirname $kafka_bin)
        kafka_config_properties=$(ps -ef | grep "broker.properties" |grep -v grep| awk '{print $NF}')
        kafka_config_real_dir=$(cd $kafka_bin && realpath $kafka_config_properties)
        kafka_config_dir=$(dirname $kafka_config_real_dir)

        kafka_bin="$kafka_dir/bin"
        echo "|       获取kafka-IP/PORT/LOG/Kraft/PORT信息             |"
        kafka_ip_member=$(cat $kafka_config_real_dir | grep listeners | grep -v ^# |grep -v "advertised"| awk -F "//" '{print $2}' | awk -F ":" '{print $1}' | grep -v '^$' | wc -l)
        if [ $kafka_ip_member == 0 ]; then
            kafka_ip=$ip
        else
            kafka_ip=$(cat $kafka_config_real_dir | grep listeners | grep -v ^# |grep -v "advertised"| awk -F "//" '{print $2}' | awk -F ":" '{print $1}' | grep -v '^$' )
        fi
        kafka_port_member=$(cat $kafka_config_real_dir | grep listeners | grep -v ^# |grep -v "advertised"| awk -F "//" '{print $2}' | awk -F ":" '{print $2}' | grep -v '^$'|wc -l)
        if [ $kafka_port_member == 0 ];then
            kafka_port=9092
        else
            kafka_port=$(cat $kafka_config_real_dir |  grep listeners | grep -v ^# |grep -v "advertised"| awk -F "//" '{print $2}' | awk -F ":" '{print $2}' | grep -v '^$')
        fi    
        ...
        echo "==============cat broer.properties================="
        cat broker.properties|grep -v ^#|grep -v ^$
        echo "==============cat controller.properties================="
        cat controller.properties|grep -v ^#|grep -v ^$
...
function kafka_kraft_security_baseline() {
    echo "|+++++++++++++++++++++++++++++++++++++++++++++++++++++++++|"
    echo "|              kafka kraft security baseline              |"
    echo "|+++++++++++++++++++++++++++++++++++++++++++++++++++++++++|"
    echo ""
    echo "当前服务器kafka进程信息!"
    for line in $(ps -ef | grep kafka.Kafka |grep broker.propertie| grep -v grep | awk -F " " '{print $2}'); do
        kafka_bin_dir=$(pwdx $line | awk -F ":" '{print $2}')
        kafka_dir=$(dirname $kafka_bin_dir)
        kafka_config_properties=$(ps -ef | grep "broker.properties" |grep -v grep| awk '{print $NF}')
        kafka_config_real_dir=$(cd $kafka_bin_dir && realpath $kafka_config_properties)
        kafka_config_dir=$(dirname $kafka_config_real_dir)
        kafka_bin="$kafka_dir/bin"
        echo "获取当前kafka的配置文件,当前获取配置的节点为$ip-$line"
        echo "基线1:端口保护;修改kafka默认监听端口9092为不易猜测的端口。"
        security_baseline_port=$(cat $kafka_config_real_dir |  grep listeners | grep -v ^# |grep -v "advertised"| awk -F "//" '{print $2}' | awk -F ":" '{print $2}' | grep -v '^$')  
        if [ $kafka_port_member == 0 ];then
        ...
        security_baseline_partitions=$(cat $kafka_config_real_dir | grep num.partitions | awk -F "=" '{print $NF}') && echo "security_baseline_partitions:$security_baseline_partitions"
        ...
        security_baseline_atuocreate=$(cat $kafka_config_real_dir | grep "auto.create.topics" | awk -F "=" '{print $NF}')
        ...
        security_baseline_logdir=$(cat $kafka_config_real_dir  | grep "log.dirs" | awk -F "=" '{print $NF}') && echo "security_baseline_logdir:$security_baseline_logdir"
        #example_dir=/tmp
...
function get_kafka_main() {
    ...
    ####get system info
    if [ `ps -ef|grep java|grep kafka|grep .properties|grep -v grep|wc -l` -eq 0 ];then
        ...
        kafka_inquiry_kraft_info >>"$filepath""$filename2"
        kafka_kraft_security_baseline >>"$filepath""$filename2"
    fi    
...
function get_kafka_data() {
    ...
    for line in $(ps -ef | grep kafka.Kafka |grep broker.properties| grep -v grep | awk -F " " '{print $2}'); do
        ...
        Broker_id=$(cat $file1 | grep "node.id" | grep -v "generation" | grep -v "sensitive")
        ...
        Check_dir_info=$(cat $file1 | sed -n /查询Kafka磁盘信息/,/运行信息查询结束/p | grep -v 查询Kafka磁盘信息 | grep -v 运行信息查询结束) && echo "\"Check_dir_info\"":"\'$Check_dir_info\'""," >>$new_document
...
function check_script_value_exist(){
    ...
    Broker_id_member=$(cat $file1 | grep "node.id" | grep -v "generation" | grep -v "sensitive"|wc -l)
