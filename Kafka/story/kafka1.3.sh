...
function check_value_quick_output(){
    ...
    do    
        ...
        #基线1:端口保护;修改kafka默认监听端口9092为不易猜测的端口。
        ...
        if [ $Kafka_Version -gt "220" ]; then
            ...
            for i in $($kafka_bin/kafka-consumer-groups.sh --zookeeper $zookeeper_ip_port --list);
            do
                I=0;
                for line in $($kafka_bin/kafka-consumer-groups.sh --zookeeper $zookeeper_ip_port --describe --group $i|awk -F " " '{print $6}'|grep -v "LAG"|sed 's/[^0-9]//g');
                do
                    I=$(($I + $line));
                done
                if [ $I -ge 5000 ];then
