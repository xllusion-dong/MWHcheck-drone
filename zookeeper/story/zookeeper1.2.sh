...
function zookeeper_inquiry_info() {
    ...
    if [ $zookeeper_pid_member == 0 ]; then
        ...
        for line in $(ps -ef | grep Dzookeeper.root | grep -v grep | awk -F " " '{print $2}'); do
            ...
            zk_dir_home=$(ps -ef|grep zookeeper|grep $line |awk -F "(-cp)" '{print $2}'|grep -v '^$'| sed 's/^[ \t]*//g'|awk -F "/bin" '{print $1}')
            ...
            cd $zk_dir_home 
            zk_config_cfg=`echo ${zk_config##*./}`
            echo "获取zookeeper的status信息和version信息"
            sh bin/zkServer.sh status  $zk_config_cfg && sh bin/zkServer.sh version $zk_config_cfg
            ...
            if [ $check_nc -eq 0 ]; then
                ...
                a=$(grep -wn "4lw.commands.whitelist" $zk_config_cfg | awk -F ':' '{print $1}')
                if [ $a ]; then
                    commands=$(cat $zk_config_cfg | grep '4lw.commands.whitelist')
                    if [ $commands = '4lw.commands.whitelist=*' ]; then
                        ...
                        commands=$(cat $zk_config_cfg | grep '4lw.commands.whitelist' | awk -F '=' '{print $2}')
...
function zookeeper_security_baseline() {
    ...
    for line in $(ps -ef | grep Dzookeeper.root | grep -v grep | awk -F " " '{print $2}'); do
        echo "获取当前zookeeper的配置文件,当前获取配置的节点为$ip-$line"
        zk_home=`pwdx $line |awk -F " " '{print $2}'`
        echo "安全基线1:端口保护;修改zookeeper默认监听端口2181为不易猜测的端口。"
        cd $zk_home && cat $zk_config 
        ...
        security_baseline_console=$(cat $zk_home/zkServer.sh | grep -i "Dzookeeper.admin.enableServer=false" | wc -l) && echo "security_baseline_console:$security_baseline_console"
        ...
        if [ $security_baseline_port -eq 2181 ]; then
            security_baseline_port1="Did not pass the security check"
        ...
        if [ $security_baseline_maxClientCnxns -le 1000 ]; then
            security_baseline_maxClientCnxns1="Pass"
            echo "security_baseline_maxClientCnxns1:$security_baseline_maxClientCnxns1" 
        else
            security_baseline_maxClientCnxns1="Did not pass the security check"
            echo "security_baseline_maxClientCnxns1:$security_baseline_maxClientCnxns1"  
        fi

        echo "安全基线3:zookeeper的授权访问。"  
        ...
        if [ $security_baseline_root == "root" ]; then
            security_baseline_root1="Did not pass the security check"
        ...
        if [ $security_baseline_dir == "root" ]; then
            security_baseline_dir1="Did not pass the security check"
        ...
        if [ $security_baseline_clientPortAddress -eq 0 ]; then
            security_baseline_clientPortAddress1="Did not pass the security check"
            echo "security_baseline_clientPortAddress1:$security_baseline_clientPortAddress1"
        else
            security_baseline_clientPortAddress1="Pass"
            echo "security_baseline_clientPortAddress1:$security_baseline_clientPortAddress1"
        fi
        echo "安全基线11:zookeeper四字命令。"
        security_baseline_4lw=$(cat $zk_config | grep '4lw.commands.whitelist' | wc -l)
        if [ $security_baseline_4lw -eq 0 ]; then
            security_baseline_4lw1="Pass"
            echo "security_baseline_4lw1:$security_baseline_4lw1"
        else
            security_baseline_4lw1="Did not pass the security check"
            echo "security_baseline_4lw1:$security_baseline_4lw1"
        fi
    done
}

function zookeeper_security_baseline_new() {
    mkdir -p /tmp/enmoResult/zookeeper/security
    security_baseline_document=/tmp/enmoResult/zookeeper/security/security_baseline.txt
    zookeeper_pids=$(ps -eo ruser,pid,args | grep java | grep -v grep | grep "org.apache.zookeeper.server.quorum.QuorumPeerMain" | awk '{print $2}')
    for zkpid in $zookeeper_pids;
    do
        zk_dir_log=$(ps -ef | grep $zkpid | grep Dzookeeper.root | grep -v grep | awk -F "-Dzookeeper.log.dir" '{print $2}' | awk -F "=" '{print $2}' | awk -F " " '{print $1}')
        dir=$(dirname $zk_dir_log)
        cd $dir && cd bin
        zk_config=$(ps -ef | grep $zkpid | grep -v grep | awk -F " " '{print $NF}')
        #zkhome=$(echo ${zk_config%%/bin*})
        echo "Zookeeper Security Baseline，The current pid is $zkpid" >> $security_baseline_document
        echo "安全基线1:端口保护;修改zookeeper默认监听端口2181为不易猜测的端口。" >> $security_baseline_document
        security_baseline_port=$(cat $zk_config | grep clientPort= | awk -F "=" '{print $2}')
        if [ $security_baseline_port -eq 2181 ]; then
            security_baseline_port1="Did not pass the security check"
            echo "security_baseline_port1:$security_baseline_port1">>$security_baseline_document
        else
            security_baseline_port1="Pass"
            echo "security_baseline_port1:$security_baseline_port1"  >> $security_baseline_document
        fi
        echo "安全基线2:预防DOS攻击;建议限制zookeeper客户端的最大连接数。"  >> $security_baseline_document
        security_baseline_maxClientCnxns=$(cat $zk_config | grep maxClientCnxns | awk -F "=" '{print $2}')
        if [ $security_baseline_maxClientCnxns -le 1000 ]; then
            security_baseline_maxClientCnxns1="Pass"
            echo "security_baseline_maxClientCnxns1:$security_baseline_maxClientCnxns1"  >> $security_baseline_document
        else
            security_baseline_maxClientCnxns1="Did not pass the security check"
            echo "security_baseline_maxClientCnxns1:$security_baseline_maxClientCnxns1"  >> $security_baseline_document
        fi
        echo "安全基线3:zookeeper的授权访问。"  >> $security_baseline_document
        security_baseline_acl=$(echo getAcl / | sh zkCli.sh -server $ip:$zookeeper_port | grep -i "world" | wc -l)
        if [ $security_baseline_acl -eq 0 ]; then
            security_baseline_acl1="Did not pass the security check" 
            echo "security_baseline_acl1:$security_baseline_acl1"  >> $security_baseline_document
        else
            security_baseline_acl1="Pass"
            echo "security_baseline_acl1:$security_baseline_acl1"  >> $security_baseline_document
        fi
        echo "安全基线4:zookeeper的日志审计。"  >> $security_baseline_document
        security_baseline_audit=$(cat $zk_config | grep -i "audit.enable=true" | wc -l)
        if [ $security_baseline_audit -eq 0 ]; then
            security_baseline_audit1="Did not pass the security check"
            echo "security_baseline_audit1:$security_baseline_audit1"  >> $security_baseline_document
        else
            security_baseline_audit1="Pass"
            echo "security_baseline_audit1:$security_baseline_audit1"  >> $security_baseline_document
        fi
        echo "安全基线5:zookeeper的事务日志与快照日志分离。"  >> $security_baseline_document
        security_baseline_log=$(cat $zk_config | grep -Ei '(dataDir|dataLogDir)' | grep -v '^#' | wc -l)
        if [ $security_baseline_log -eq 2 ]; then
            security_baseline_log1="Pass"
            echo "security_baseline_log1:$security_baseline_log1"  >> $security_baseline_document
        else
            security_baseline_log1="Did not pass the security check"
            echo "security_baseline_log1:$security_baseline_log1"  >> $security_baseline_document
        fi
        echo "安全基线6:zookeeper的日志清理。"  >> $security_baseline_document
        security_baseline_clean=$(cat $zk_config | grep -i "autopurge" | wc -l)
        if [ $security_baseline_clean -eq 0 ]; then
            security_baseline_clean1="Did not pass the security check"
            echo "security_baseline_clean1:$security_baseline_clean1"  >> $security_baseline_document
        else
            security_baseline_clean1="Pass"
            echo "security_baseline_clean1:$security_baseline_clean1"  >> $security_baseline_document
        fi
...
function get_zookeeper_data() {
    ...
    Installation_path=$(cat $file1 | grep "org.apache.zookeeper.server.quorum.QuorumPeerMain"|awk -F "(-cp)" '{print $2}'|grep -v '^$'| sed 's/^[ \t]*//g'|awk -F "/bin" '{print $1}') && echo "\"Installation_path\"":"\"$Installation_path\"""," >>$new_document
    ...
    JVM_Instal_path=$(cat $file1 | grep -A1 "当前服务器zookeeper进程信息" | grep -v "当前服务器zookeeper进程信息" | grep -v grep |grep "org.apache.zookeeper.server.quorum.QuorumPeerMain" | awk -F " " '{print $8}') && echo "\"JVM_Instal_path\"":"\"$JVM_Instal_path\"""," >>$new_document
    JVM_run_parameters=$(cat $file1 |grep "org.apache.zookeeper.server.quorum.QuorumPeerMain"| egrep -io 'Xmx.*|Xms.*' | awk -F "-Djava" '{print $1}') && echo "\"JVM_run_parameters\"":"\"$JVM_run_parameters\"""," >>$new_document
...
zookeeper_security_baseline_new
cd $data_path
