#!/bin/bash
#version:1.1
#20221026 add nginx include conf data
...
function nginx_inquiry_info(){
    ...
    do
        ...
        nginx_conf=`ps -ef|grep $NPID|grep master|grep -v grep|awk -F "-c" '{print $2}'|tr -d " "`
        ...
        accesslog=`cat $nginx_conf|grep -vE '^#|^\s*#|^$'|grep access_log|grep -v off|awk '{print $2}'|tr -d ";"`
        errorlog=`cat $nginx_conf|grep -vE '^#|^\s*#|^$'|grep error_log|grep -v off|awk '{print $2}'|tr -d ";"`
        if [ -n "$errorlog" ];then
            if [[ $errorlog =~ ^/.* ]];then
                error_log=$errorlog
            else
        ...
        echo "######$nginx_conf Info:" > /tmp/enmoResult/tmpcheck/nginx_temp.xml
        nginx_temp=/tmp/enmoResult/tmpcheck/nginx_temp.xml
        cat $nginx_conf|grep -vE '^#|^\s*#|^$' >> $nginx_temp
        includes=`cat $nginx_conf|grep -vE '^#|^\s*#|^$'|grep "include "|sed 's/;/ /g'`
        for include in $includes
        do
            if echo "$include"|grep -q -E '\.conf$'; then
                if [[ $include =~ ^/.* ]];then
                    echo "######$include Info:" >> $nginx_temp
                    cat $include|grep -vE '^#|^\s*#|^$' >> $nginx_temp
                else
                    echo "######$nginx_dir/conf/$include Info:" >> $nginx_temp
                    cat $nginx_dir"/conf/"$include|grep -vE '^#|^\s*#|^$' >> $nginx_temp
                fi
            fi
        done
        
        port=`cat $nginx_temp|grep -vE '^#|^\s*#|^$'|grep "listen "|awk '{print $NF}'|tr -d ";"`
        a=":"
        if [[ "$port" =~ "$a" ]];then
        ...
        echo "Process $NPID Nginx Listen Port:"$listen_port
        ...
        # 版本隐藏配置
        server_token=`cat $nginx_temp|grep -v '^#'|grep "server_tokens"|sed 's/^[ \t]*//g'`
        if [ -n "$server_token" ];then
            ...
            echo "Process $NPID Nginx ServerToken Check Result:server_tokens on"
            echo "Process $NPID Nginx ServerToken Check Conclusion:Failed"
        fi
        
        # 禁止目录浏览
        dir_brow=`cat $nginx_temp|grep -v '^#'|grep "autoindex "|sed 's/^[ \t]*//g'`
        if [ -n "$dir_brow" ];then
            ...
            echo "Process $NPID Nginx Directory Check Result:autoindex off"
            echo "Process $NPID Nginx Directory Check Conclusion:Pass"
        fi
        
        # 连接超时设置
        time_out=`cat $nginx_temp|grep -v '^#'|grep timeout|sed 's/^[ \t]*//g'`
        echo "Process $NPID Nginx TimeOutCheckInfo"
        echo "$time_out"
        echo "Process $NPID Nginx TimeOutCheckInfoEND"
        if [ -n "$time_out" ];then
        ...
        # 自定义错误页面
        error_page=`cat $nginx_temp|grep -v '^#'|grep "error_page"|sed 's/^[ \t]*//g'`
        echo "Process $NPID Nginx ErrorPagesCheckInfo"
        echo "$error_page"
        echo "Process $NPID Nginx ErrorPagesCheckInfoEND"
...
function get_nginx_data(){
    ...
    errorpage_check=`cat $file1 |sed -n /ErrorPagesCheckInfo/,/ErrorPagesCheckInfoEND/p|grep -v "ErrorPagesCheckInfo"|grep -v "ErrorPagesCheckInfoEND"`
    if [ -n "$errorpage_check" ];then
        echo "\"errorpage_check\"":"\"$errorpage_check\"""," >> $new_document
    else
        echo "\"errorpage_check\"":"\"Null-未配置错误页面\"""," >> $new_document
    fi
    ...
    timeout_check=`cat $file1 |sed -n /TimeOutCheckInfo/,/TimeOutCheckInfoEND/p|grep -v "TimeOutCheckInfo"|grep -v "TimeOutCheckInfoEND"`
    if [ -n "$timeout_check" ];then
        echo "\"timeout_check\"":"\"$timeout_check\"""," >> $new_document
    else
        echo "\"timeout_check\"":"\"Null-未配置连接超时\"""," >> $new_document
    fi
