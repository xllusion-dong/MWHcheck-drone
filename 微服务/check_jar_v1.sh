...
#获取server运行状态信息
function get_jar_main() {

    init=0
    PIDCOUNT=$(ps -eo ruser,pid,args | grep "java" | grep jar | grep -v grep | wc -l)
    PID=$(ps -eo ruser,pid,args | grep "java" | grep jar | grep -v grep | awk ' { print $2 }')
    ...
    for OPID in $PID; do
        ...
        echo "{"
        jarcommandinfo=$(ps -p $OPID -o args | grep -v COMMAND)
        echo "\"jarcommandinfo\"":"\"$jarcommandinfo\""","
        ...
        if [ x$java_bin = x'/usr/bin/java' ];then
            ...
            javaversion=$($java_bin -version 2>&1 |awk 'NR==1 {gsub(/"/,""); print $3}')
            echo "\"javaversion\"":"\"$javaversion\""","
            ###后续更新使用oracle jdk收集时使用jstack、jcmd、jstat等命令
        fi


        openjarcount=$(ps  -Lf $OPID |wc -l)
        jarresourceinfo=$(top -bn3 | grep  $OPID)
        jarlistenportinfo=$(netstat -nutlp | grep $OPID)

        echo "\"openjarcount\"":"\"$openjarcount\""","
        echo "\"jarresourceinfo\"":"\"$jarresourceinfo\""","
        echo "\"jarlistenportinfo\"":"\"$jarlistenportinfo\""","
...
function get_os_main() {
    ...
    filename2=$HOSTNAME"_"jar_""$ipinfo"_"$qctime".txt"
...
function get_jar_json_main(){

    echo \"jarinfo\": [ >>$new_document
    get_jar_main >>$new_document
    echo ] >>$new_document
}


function mwcheckr_result_jsondata() {
    new_document=$new_data/jar_$ipinfo.json
    echo "{" >$new_document
...
mkdir -p /tmp/enmoResult/jar
new_data=/tmp/enmoResult/jar
